...
class holy_core(pluginTemplate):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.dut_exe = os.path.join(config['PATH'] if 'PATH' in config else "","holy_core")
    ...
    def build(self, isa_yaml, platform_yaml):
      ...
      # for holy_core start building the '--isa' argument. the self.isa is dutnmae specific and may not be
      # useful for all DUTs
    ...
    def runTests(self, testList):
	  ...
	  # echo statement.
          if self.target_run:
            # set up the simulation command. Template is for holy_core. Please change.
...
#
#          if self.target_run:
#            # build the command for running the elf on the DUT. In this case we use holy_core and indicate
#            # the isa arg that we parsed in the build stage, elf filename and signature filename.
#            # Template is for holy_core. Please change for your DUT
#            execute = self.dut_exe + ' --isa={0} +signature={1} +signature-granularity=4 {2}'.format(self.isa, sig_file, elf)
#            logger.debug('Executing on holy_core ' + execute)
#
